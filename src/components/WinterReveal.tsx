...
export function WinterReveal({ station, onBack }: { station: Station; onBack: () => void }) {
      ...
      <div className="paper-grain min-h-dvh" style={{ backgroundColor: `${station.colors.wash}66` }}>
        <div className="mx-auto grid min-h-dvh w-full max-w-[1180px] lg:grid-cols-[0.92fr_1.08fr]">
          <div className="relative min-h-[42dvh] overflow-hidden border-b border-[var(--line)] lg:min-h-dvh lg:border-b-0 lg:border-r">
            ...
            <div className="frost-clear pointer-events-none absolute inset-0" aria-hidden="true" />
          ...
          <div className="flex flex-col justify-between px-6 pb-8 pt-8 sm:px-9 lg:px-14 lg:py-12">
            <div>
              <div className="flex items-center gap-4">
                ...
                <p className="eyebrow winter-content" style={{ color: station.colors.accent }}>{station.number} / {station.name}</p>
              </div>
              <h1 id={`reveal-${station.key}`} className="serif winter-content mt-8 text-[clamp(4rem,9vw,7rem)] leading-[0.84] tracking-[-0.055em]" style={{ color: station.colors.ink }}>
                <span className="mr-4 italic" style={{ color: station.colors.accent }}>{station.number}</span>{station.name}
              </h1>
              <p className="eyebrow winter-content-2 mt-10" style={{ color: station.colors.accent }}>Para abrir este momento</p>
              <h2 className="serif winter-content-2 mt-4 max-w-[480px] text-[clamp(2.4rem,5vw,4rem)] leading-[0.9] tracking-[-0.045em]" style={{ color: station.colors.ink }}>{station.gift}</h2>
              <p className="serif winter-content-2 mt-6 max-w-[470px] text-[1.65rem] leading-[1.08] text-[var(--ink)] sm:text-[2rem]">“{station.message}”</p>
              <p className="winter-content-2 mt-6 max-w-[410px] text-[0.92rem] leading-7 text-[var(--muted)]">{station.detail}</p>
            </div>
